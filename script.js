...
const updateGame = (p1, p2, gameState) => {
  ...
  if (p1.health <= 0 || p2.health <= 0) {
    ...
    gameState.declareWinner(gameState.isOver, p1, p2)
  }
}
...
console.log(`${player.name} damaged ${enemy.name} by ${random}`);
  }
...
updateGame(player,p2,game)
    //  Return a message of 'player name heals for hpAmount HP'
console.log(`${player.name} healed by ${random}`);
}
}
...
class Game {
  ...
  declareWinner(isOver, p1, p2) {
    ...
    if (isOver == true && p1.health <= 0) {
      message = `${p2.name} defeated ${p1.name}`
    ...
    // Play victory sound
      ...
      message = `${p1.name} defeated ${p2.name}`
      resultDiv.innerText = message
      document.getElementById('victory').play()
    }
}
...
let player1 = new Player('Ritwik', 100, 10)
let player2 = new Player('Kuku', 100, 10)
